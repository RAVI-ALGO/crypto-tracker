...
const CurrencyTable = (prop) => {
  ...
  const columns = [
    {
      ...
      selector: (row) => (
        <div className="flex">
          <button className="border-0 outline-0 bg-none cursor-pointer">
            <svg
            className="fill-gray-500 hover:fill-cyan-400"
              width="20"
              height="20"
              viewBox="0 0 30 30"
              fill="cyan"
          ...
          {/* <img height="25px" className="mx-2" width="25px" src={row.image} /> */}
          <span className=" ml-2">{row.symbol}</span>{" "}
        </div>
      ),
    },
    {
    ...
    {
      name: "Price",
      selector: (row) =><span>{(new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'usd' }).format(row.price_usd))}</span> ,
      sortable: true,
    },
    {
      name: "Total Volume",
      selector: (row) => row.tsupply,
      sortable: true,
    },
    {
      name: "Market Cap",
    ...
    {
      name: "1D",
      selector: (row) => <span className={row.percent_change_1h >0 ?"text-green-500" :"text-red-500"}>{Number(row.percent_change_1h).toFixed(2)} %</span>,
    },
    {
      name: "24H",
      selector: (row) => <span className={row.percent_change_24h >0 ?"text-green-500" :"text-red-500"}>{Number(row.percent_change_24h).toFixed(2)} %</span>,
      
    },
    {
      name: "7D",
      selector: (row) =><span className={row.percent_change_7d >0 ?"text-green-500" :"text-red-500"}>{Number(row.percent_change_7d).toFixed(2)} %</span>,
    },
  ];
  ...
  return (
    <div className="table-main w-[80%] mx-auto flex flex-col   text-gray-200">
      <div className="table-controls flex justify-between  border mt-5 mx-5 border-cyan-300">
        <span className=" text-xl py-3 mx-auto">{prop.pagename}</span>
        {/* seachbar */}
        {/* <form className="px-4">
          <div className="relative mb-4 mt-3 ml-3 flex w-full flex-wrap items-stretch">
            <input
              type="search"
              className="relative m-0 block w-[30%] min-w-0 flex-auto rounded border border-solid border-cyan-300 bg-transparent bg-clip-padding px-3 py-[0.25rem] text-base font-normal leading-[1.6] text-gray-200 outline-none transition duration-200 ease-in-out focus:z-[3] focus:border-primary focus:text-gray-200 focus:shadow-[inset_0_0_0_1px_rgb(59,113,202)] focus:outline-none dark:border-neutral-600 dark:text-gray-200 dark:placeholder:text-gray-200 dark:focus:border-primary"
            ...
            {/* <!--Search icon--> */}
            {/* <button
              className="input-group-text flex items-center whitespace-nowrap rounded px-3 py-1.5 text-center text-base font-normal text-white dark:text-gray-200"
              id="basic-addon2"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        {/* <div className="relative mb-4 px-4 my-3 flex flex-wrap items-stretch">
          <select
            id="#currency"
            data-te-select-init
            className=" px-4 border   rounded border-cyan-300 bg-transparent text-gray-200 outline-none transition duration-200 ease-in-out focus:z-[3] focus:border-cyan-300 focus:text-gray-200 focus:bg-transparent  focus:outline-none dark:border-cyan-300 "
          >
            <option className="" value=" ">
              Change Currency
            </option>
            <option className="" value="usdt">
              USDT
            </option>
        ...
        {/* <div className="relative mb-4 px-4 my-3 flex flex-wrap items-stretch">
          <button
            className="input-group-text flex items-center whitespace-nowrap rounded px-3 py-1.5 text-center text-base font-normal text-cyan-300 dark:text-gray-200"
            id="basic-addon2"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
      ...
      <div className="table-content  border  my-5 mx-5 border-cyan-300">
        <DataTable
          columns={columns}
          data={prop.cryptoData}
          pagination
          selectableRows
          responsive
          pointerOnHover
